#PPNC version 3 - Corrected initial conditions
import time
import math 
import numpy as np
import pylab as p
...
def PPNC2(z, H0, Omega_M0, k, alpha_0, p, gamma_0, gamma_cm0, q, gamma_cDE0, r):
    ...
    zs = np.linspace(0.0, z_final, 500.0)
    
    # The scale factor for different redshifts
    a_vals = 1.0/(1.0+zs)
    #a_vals = np.linspace(1.0, a_final, 500)
    #zs = (1.0/a_vals) - 1.0
    ...
    def gamma_cDEf(a):
        #print('gamma_cDE', gamma_cDE0*(a**(-r)))
    ...
    def con_Hf(a):
        return ((H0*H0*Omega_M0*gammaf(a))/a - 2.0*gamma_cf(a)*(a*a)/3.0 - k)**(0.5)
    ...
    def con_H_primef(a): 
        ...
        return (-(H0*H0*Omega_M0*alphaf(a))/(2.0*a) + alpha_c*(a*a)/3.0)

    #Define function to evaluate rate of change of growth factor D
    def dD_da(D, a, alphaf, con_Hf, con_H_primef):
    ...
    D0 = np.array([1.0, 0.512781029758571]) #Fixed using CCL growth rate today
    
    #integrate to obtain growth factor solutions
    Ds = odeint(dD_da, D0, a_vals, args = (alphaf, con_Hf, con_H_primef))
...
# Using H0 = 73.24 km/s/Mpc, H0 in units of G=c=1
...
P.figure(5)
#Plot growth rate
P.plot(1./(1. + z), (f - ccl_f)/ccl_f, 'b-', lw=1.8)
#P.plot(1./(1. + z), ccl_f, 'c--', lw=1.8)
P.plot(1./(1. + z), (D[:,0] - ccl_D)/ccl_D, 'r-', lw=1.8)
#P.plot(1./(1. + z), ccl_D, 'b--', lw=1.8)
#np.set_printoptions(precision = 15)
